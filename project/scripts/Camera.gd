...
@export var KEY_ESCAPE: String = "ui_cancel" # Changed to use built-in escape action
...
func _ready():
	...
	if camera == null:
		camera = Camera3D.new()
		add_child(camera)
		camera.position = Vector3(0, 1.5, 0) # Set default camera position at eye level
...
func _process(delta: float) -> void:
	if !UPDATE_ON_PHYSICS:
		handle_input(delta) # Handle player input
...
func handle_input(_delta):
	...
	if Input.is_action_just_pressed(KEY_ESCAPE):
		...
		return # Don't process movement when mouse is released
