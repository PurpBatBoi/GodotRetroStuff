...
const POINT_MATERIAL := "rls_point_light_icon"
const DIRECTIONAL_MATERIAL := "rls_directional_light_icon"
const SPOT_MATERIAL := "rls_spot_light_icon"
const LINES_PRIMARY := "rls_lines_primary"
const LINES_SECONDARY := "rls_lines_secondary"
const LINES_BILLBOARD := "rls_lines_billboard"
const HANDLES := "rls_handles"
const HANDLES_BILLBOARD := "rls_handles_billboard"
...
func _has_gizmo(for_node_3d: Node3D) -> bool:
	return for_node_3d != null and (
		for_node_3d.is_class("RLS_PointLight3D") or
		for_node_3d.is_class("RLS_DirectionalLight3D") or
		for_node_3d.is_class("RLS_SpotLight3D")
	)
...
func _redraw(gizmo: EditorNode3DGizmo) -> void:
	...
	if node.is_class("RLS_PointLight3D"):
		icon_material = get_material(POINT_MATERIAL, gizmo)
	elif node.is_class("RLS_DirectionalLight3D"):
		icon_material = get_material(DIRECTIONAL_MATERIAL, gizmo)
	elif node.is_class("RLS_SpotLight3D"):
	...
	if node.is_class("RLS_DirectionalLight3D"):
	...
	if node.is_class("RLS_SpotLight3D") and _is_node_selected(node):
		var r: float = node.get_range()
...
func _get_handle_name(gizmo: EditorNode3DGizmo, handle_id: int, secondary: bool) -> String:
	...
	if handle_id == HANDLE_RANGE:
		return "Radius"
	if node.is_class("RLS_SpotLight3D") and handle_id == HANDLE_SPOT_ANGLE:
		return "Aperture"
	return ""
...
func _get_handle_value(gizmo: EditorNode3DGizmo, handle_id: int, secondary: bool) -> Variant:
	...
	if handle_id == HANDLE_RANGE and (node.is_class("RLS_PointLight3D") or node.is_class("RLS_SpotLight3D")):
		return node.get_range()
	if handle_id == HANDLE_SPOT_ANGLE and node.is_class("RLS_SpotLight3D"):
...
func _set_handle(gizmo: EditorNode3DGizmo, handle_id: int, secondary: bool, camera: Camera3D, point: Vector2) -> void:
	...
	if node.is_class("RLS_SpotLight3D"):
...
func _commit_handle(gizmo: EditorNode3DGizmo, handle_id: int, secondary: bool, restore: Variant, cancel: bool) -> void:
	...
	if handle_id == HANDLE_RANGE and (node.is_class("RLS_PointLight3D") or node.is_class("RLS_SpotLight3D")):
		property_name = "range"
		action_name = "Change RLS Light Radius"
		current_value = node.get_range()
	elif handle_id == HANDLE_SPOT_ANGLE and node.is_class("RLS_SpotLight3D"):
		property_name = "spot_angle"
		action_name = "Change RLS Spot Aperture"
