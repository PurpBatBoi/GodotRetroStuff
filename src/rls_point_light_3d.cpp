...
RLS_PointLight3D::RLS_PointLight3D() {
	set_notify_transform(true);
}

void RLS_PointLight3D::_bind_methods() {
	ClassDB::bind_method(D_METHOD("set_enabled", "enabled"), &RLS_PointLight3D::set_enabled);
	ClassDB::bind_method(D_METHOD("is_enabled"), &RLS_PointLight3D::is_enabled);
	ClassDB::bind_method(D_METHOD("set_color", "color"), &RLS_PointLight3D::set_color);
	ClassDB::bind_method(D_METHOD("get_color"), &RLS_PointLight3D::get_color);
	ClassDB::bind_method(D_METHOD("set_energy", "energy"), &RLS_PointLight3D::set_energy);
	ClassDB::bind_method(D_METHOD("get_energy"), &RLS_PointLight3D::get_energy);
	ClassDB::bind_method(D_METHOD("set_range", "range"), &RLS_PointLight3D::set_range);
...
void RLS_PointLight3D::_notification(int p_what) {
	switch (p_what) {
...
void RLS_PointLight3D::set_enabled(bool p_enabled) {
...
bool RLS_PointLight3D::is_enabled() const {
	return enabled;
}

void RLS_PointLight3D::set_color(const Color &p_color) {
	if (color == p_color) {
		return;
	}
	color = p_color;
...
Color RLS_PointLight3D::get_color() const {
	return color;
}

void RLS_PointLight3D::set_energy(float p_energy) {
...
float RLS_PointLight3D::get_energy() const {
	return energy;
}

void RLS_PointLight3D::set_range(float p_range) {
...
float RLS_PointLight3D::get_range() const {
	return range;
}

void RLS_PointLight3D::set_attenuation(float p_attenuation) {
...
bool RLS_PointLight3D::is_fake_point_light() const {
	return fake_point_light;
}

void RLS_PointLight3D::_reconnect_manager() {
