...
Color get_node_color(Node3D *p_node) {
	if (const RLS_PointLight3D *point = Object::cast_to<RLS_PointLight3D>(p_node)) {
		return point->is_enabled() ? point->get_color() : point->get_color().darkened(0.45);
	}
	if (const RLS_DirectionalLight3D *directional = Object::cast_to<RLS_DirectionalLight3D>(p_node)) {
		return directional->is_enabled() ? directional->get_color() : directional->get_color().darkened(0.45);
	}
	if (const RLS_SpotLight3D *spot = Object::cast_to<RLS_SpotLight3D>(p_node)) {
...
void RLSLightGizmoPlugin::_bind_methods() {
}

void RLSLightGizmoPlugin::set_editor_plugin(EditorPlugin *p_editor_plugin) {
	editor_plugin = p_editor_plugin;
}

void RLSLightGizmoPlugin::_ensure_materials() {
...
void RLSLightGizmoPlugin::_commit_property_change(Object *p_object, const String &p_action_name, const StringName &p_property, const Variant &p_do_value, const Variant &p_undo_value) const {
	if (p_object == nullptr) {
		return;
	}
...
bool RLSLightGizmoPlugin::_has_gizmo(Node3D *p_for_node_3d) const {
	return Object::cast_to<RLS_PointLight3D>(p_for_node_3d) != nullptr ||
			Object::cast_to<RLS_DirectionalLight3D>(p_for_node_3d) != nullptr ||
			Object::cast_to<RLS_SpotLight3D>(p_for_node_3d) != nullptr;
}

String RLSLightGizmoPlugin::_get_gizmo_name() const {
	return "RLSLights";
}

int32_t RLSLightGizmoPlugin::_get_priority() const {
	return -1;
}

void RLSLightGizmoPlugin::_redraw(const Ref<EditorNode3DGizmo> &p_gizmo) {
	if (p_gizmo.is_null()) {
		return;
	}
	...
	if (Object::cast_to<RLS_PointLight3D>(node) != nullptr) {
		material = get_material(String(POINT_MATERIAL_NAME), p_gizmo);
	} else if (Object::cast_to<RLS_DirectionalLight3D>(node) != nullptr) {
		material = get_material(String(DIRECTIONAL_MATERIAL_NAME), p_gizmo);
	} else if (Object::cast_to<RLS_SpotLight3D>(node) != nullptr) {
	...
	if (RLS_PointLight3D *point_light = Object::cast_to<RLS_PointLight3D>(node)) {
	...
	if (Object::cast_to<RLS_DirectionalLight3D>(node) != nullptr) {
		PackedVector3Array lines;
	...
	if (RLS_SpotLight3D *spot_light = Object::cast_to<RLS_SpotLight3D>(node)) {
		PackedVector3Array lines;
...
String RLSLightGizmoPlugin::_get_handle_name(const Ref<EditorNode3DGizmo> &p_gizmo, int32_t p_handle_id, bool p_secondary) const {
	...
	if (Object::cast_to<RLS_SpotLight3D>(node) != nullptr) {
...
Variant RLSLightGizmoPlugin::_get_handle_value(const Ref<EditorNode3DGizmo> &p_gizmo, int32_t p_handle_id, bool p_secondary) const {
	Node3D *node = p_gizmo.is_valid() ? p_gizmo->get_node_3d() : nullptr;
	if (node == nullptr || p_secondary) {
		return Variant();
	}

	if (const RLS_PointLight3D *point_light = Object::cast_to<RLS_PointLight3D>(node)) {
		if (p_handle_id == HANDLE_RANGE) {
			return point_light->get_range();
		}
	}

	if (const RLS_SpotLight3D *spot_light = Object::cast_to<RLS_SpotLight3D>(node)) {
		if (p_handle_id == HANDLE_RANGE) {
...
void RLSLightGizmoPlugin::_set_handle(const Ref<EditorNode3DGizmo> &p_gizmo, int32_t p_handle_id, bool p_secondary, Camera3D *p_camera, const Vector2 &p_screen_pos) {
	Node3D *node = p_gizmo.is_valid() ? p_gizmo->get_node_3d() : nullptr;
...
void RLSLightGizmoPlugin::_commit_handle(const Ref<EditorNode3DGizmo> &p_gizmo, int32_t p_handle_id, bool p_secondary, const Variant &p_restore, bool p_cancel) {
	Node3D *node = p_gizmo.is_valid() ? p_gizmo->get_node_3d() : nullptr;
	if (node == nullptr || p_secondary) {
		return;
	}
	...
	if (RLS_PointLight3D *point_light = Object::cast_to<RLS_PointLight3D>(node)) {
		...
		if (!Math::is_equal_approx(current, restore)) {
			_commit_property_change(point_light, "Change RLS Point Light Range", StringName("range"), current, restore);
		}
		return;
	}
	...
	if (RLS_SpotLight3D *spot_light = Object::cast_to<RLS_SpotLight3D>(node)) {
		if (p_handle_id == HANDLE_RANGE) {
			...
			if (!Math::is_equal_approx(current, restore)) {
				_commit_property_change(spot_light, "Change RLS Spot Light Range", StringName("range"), current, restore);
			}
			return;
		}
		...
		if (p_handle_id == HANDLE_SPOT_ANGLE) {
			...
			if (!Math::is_equal_approx(current, restore)) {
				_commit_property_change(spot_light, "Change RLS Spot Light Angle", StringName("spot_angle"), current, restore);
			}
		}
	}
}
